...
pub struct ClientConfig {
	...
	/// Will take the key from env. (if .api_key is None)
	pub api_key_from_env: Option<EnvName>,
...
impl ClientConfig {
	...
	pub fn key_from_env(mut self, key_from_env: impl Into<EnvName>) -> Self {
		self.api_key_from_env = Some(key_from_env.into());
		self
	}
...
impl fmt::Debug for ClientConfig {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		f.debug_struct("ClientConfig")
			.field("key_from_env", &self.api_key_from_env)
			.field("key", &self.api_key.as_ref().map(|_| "REDACTED"))
...
impl Default for ClientConfig {
	fn default() -> Self {
		Self {
			api_key_from_env: Some(EnvName::AdapterDefault),
			api_key: None,
			endpoint: None,
		}
	}
